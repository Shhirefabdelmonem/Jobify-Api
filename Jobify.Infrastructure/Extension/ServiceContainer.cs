...
namespace Jobify.Infrastructure.Extention
{
    public static class ServiceContainer
    {
        ...
        private static IServiceCollection AddServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            // Register RecommendationSettings
            var recommendationSettings = configuration.GetSection("RecommendationSettings").Get<RecommendationSettings>()
                ?? new RecommendationSettings();
            services.AddSingleton(recommendationSettings);

            // Register HttpClient for RecommendationService with proper configuration
            services.AddHttpClient<IRecommendationService, RecommendationService>(client =>
            {
                client.BaseAddress = new Uri(recommendationSettings.BaseUrl);
                client.Timeout = TimeSpan.FromSeconds(recommendationSettings.TimeoutSeconds);
                client.DefaultRequestHeaders.Add("User-Agent", "Jobify-API/1.0");
            });
